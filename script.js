...
function generateDisposalMPR() {
    const fromDate = new Date(document.getElementById('disposalMprFromDate').value);
    const toDate = new Date(document.getElementById('disposalMprToDate').value);
    ...
    const periodDisposals = disposals.filter(d => {
        const dispDate = new Date(d.disposalDate);
        return dispDate >= fromDate && dispDate <= toDate;
    });

    let over5Yrs = 0, over3Yrs = 0, over2Yrs = 0, over1Yr = 0, lessThan1Yr = 0;
    const detailList = [];

    periodDisposals.forEach(d => {
        const regDate = new Date(d.regDate);
        const dispDate = new Date(d.disposalDate);
        const pendingDays = Math.ceil((dispDate - regDate) / (1000 * 60 * 60 * 24));
        const pendingYears = pendingDays / 365.25;

        let ageCategory = '';
        if (pendingYears > 5) { over5Yrs++; ageCategory = 'Over 5 Years'; }
        else if (pendingYears > 3) { over3Yrs++; ageCategory = 'Over 3 Years'; }
        else if (pendingYears > 2) { over2Yrs++; ageCategory = 'Over 2 Years'; }
        else if (pendingYears > 1) { over1Yr++; ageCategory = 'Over 1 Year'; }
        else { lessThan1Yr++; ageCategory = 'Less Than 1 Year'; }

        detailList.push({
            rrcms: d.rrcms, stream: d.stream, regDate: d.regDate,
            disposalDate: d.disposalDate, pendingDays: pendingDays,
            ageCategory: ageCategory, disposalType: d.disposalType, remarks: d.remarks || '-'
        });
    });

    const totalDisposal = periodDisposals.length;

    document.getElementById('disposalMprSummary').style.display = 'block';
    document.getElementById('disposalMprPeriod').textContent = 
        formatDate(document.getElementById('disposalMprFromDate').value) + ' to ' + 
        formatDate(document.getElementById('disposalMprToDate').value);
    document.getElementById('disposalMprTotal').textContent = totalDisposal;

    const tbody = document.getElementById('disposalMprTableBody');
    tbody.innerHTML = `<tr>
        <td>1</td>
        <td style="text-align:left;font-weight:600;">${officeName}</td>
        <td style="font-weight:700;color:#2c5282;font-size:1.1rem;">${totalDisposal}</td>
        <td style="color:#e53e3e;font-weight:600;">${over5Yrs}</td>
        <td style="color:#dd6b20;font-weight:600;">${over3Yrs}</td>
        <td style="color:#d69e2e;font-weight:600;">${over2Yrs}</td>
        <td style="color:#38a169;font-weight:600;">${over1Yr}</td>
        <td style="color:#3182ce;font-weight:600;">${lessThan1Yr}</td>
    </tr>`;

    const detailBody = document.getElementById('disposalMprDetailBody');
    const detailSection = document.getElementById('disposalMprDetailSection');

    if (detailList.length > 0) {
        detailSection.style.display = 'block';
        detailList.sort((a, b) => b.pendingDays - a.pendingDays);
        detailBody.innerHTML = detailList.map((d, i) => {
            let ageBadgeClass = 'badge-green';
            if (d.ageCategory === 'Over 5 Years') ageBadgeClass = 'badge-red';
            else if (d.ageCategory === 'Over 3 Years') ageBadgeClass = 'badge-red';
            else if (d.ageCategory === 'Over 2 Years') ageBadgeClass = 'badge-orange';
            else if (d.ageCategory === 'Over 1 Year') ageBadgeClass = 'badge-yellow';
            return `<tr>
                <td>${i + 1}</td>
                <td><strong>${d.rrcms}</strong></td>
                <td><span class="stream-tag" style="background:${getStreamColor(d.stream)}20;color:${getStreamColor(d.stream)}">${d.stream}</span></td>
                ...
                <td>${formatDate(d.disposalDate)}</td>
                <td>${d.pendingDays}</td>
                <td><span class="badge ${ageBadgeClass}">${d.ageCategory}</span></td>
                <td><span class="badge badge-green">${d.disposalType}</span></td>
                <td>${d.remarks}</td>
            </tr>`;
        }).join('');
    ...
    showToast('Disposal MPR Report generated successfully!');
}

function exportDisposalMPRCSV() {
    const tbody = document.getElementById('disposalMprTableBody');
    if (!tbody.innerHTML.trim()) {
        showToast('Please generate Disposal MPR first!', 'error');
        return;
    }
    ...
    let csv = `Disposal MPR Report - ${fromDate} to ${toDate}\n\n`;
    csv += 'S.No.,Office Name,Total Disposal,Over 5 Years,Over 3 Years,Over 2 Years,Over 1 Year,Less Than 1 Year\n';
    const rows = tbody.querySelectorAll('tr');
    rows.forEach(row => {
        const cells = row.querySelectorAll('td');
        const values = Array.from(cells).map(c => c.textContent.trim());
    ...
    if (detailBody.innerHTML.trim()) {
        csv += '\n\nDetail List:\n';
        csv += 'S.No.,RRCMS No.,Stream,Reg Date,Disposal Date,Pending Days,Age Category,Disposal Type,Remarks\n';
        const detailRows = detailBody.querySelectorAll('tr');
