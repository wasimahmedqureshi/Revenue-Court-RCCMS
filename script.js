...
function generateDisposalMPR() {
    ...
    // Filter disposals within selected period
    const periodDisposals = disposals.filter(d => {
        const dispDate = new Date(d.disposalDate);
        return dispDate >= fromDate && dispDate <= toDate;
    });

    // Categorize by age (pending days at time of disposal)
    ...
    periodDisposals.forEach(d => {
        ...
        if (pendingYears > 5) {
            over5Yrs++;
            ageCategory = 'Over 5 Years';
        } else if (pendingYears > 3) {
            over3Yrs++;
            ageCategory = 'Over 3 Years';
        } else if (pendingYears > 2) {
            over2Yrs++;
            ageCategory = 'Over 2 Years';
        } else if (pendingYears > 1) {
            over1Yr++;
    ...
    if (detailList.length > 0) {
        ...
        detailList.sort((a, b) => b.pendingDays - a.pendingDays); // Sort by pending days descending
...
';
    ...
    // Add detail section if available
    const detailBody = document.getElementById('disposalMprDetailBody');
